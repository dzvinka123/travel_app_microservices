...
def add_travel_card_task(card: dict):
    result = travel_cards.insert_one({
        "from": card["from_"], "to": card["to"],
        "start_date": card["start_date"], "end_date": card["end_date"],
        "description": card["description"], "active": card["active"]
    })
    card_id = str(result.inserted_id)

    if card["emails"]:
        user_card.insert_many([
            {"user_email": email, "card_id": card_id}
